...
import com.outils.PdfListChallenge;
 ...
 * @author Sameh
 */
public class ListeChallangefxmlController implements Initializable {
 private ObservableList<Challenge> ChallengeData;

    public ObservableList<Challenge> getCategorieData() {
    ...
    private Button   genButton;
         @FXML
    private TableView< Challenge> NotifTable;
    @FXML
    private TableColumn< Challenge, String> NameNotfi;
    @FXML
    private TableColumn< Challenge, String> dateNotif;    
      @FXML
    private TableColumn< Challenge, String>  descNotif  ;
         @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
      NameNotfi.setCellValueFactory(new PropertyValueFactory< Challenge, String>("nom"));
        dateNotif.setCellValueFactory(new PropertyValueFactory< Challenge, String>("dateChallenge"));
        descNotif.setCellValueFactory(new PropertyValueFactory< Challenge, String>("description"));
        ChallengeData= FXCollections.observableArrayList();
         NotifTable.setItems(ChallengeData);
       Task<ObservableList<Challenge>> task = new ListeChallangefxmlController .GetDailySalesTask();
     ...
     public class GetDailySalesTask extends Task<ObservableList<Challenge>> {       
        @Override protected ObservableList<Challenge> call() throws Exception {
            for (int i = 0; i < 200; i++) {
          ...
          ChallengeData.addAll(list);            
            return ChallengeData;
        }
    }
     public class GetListCatByNom extends Task<ObservableList<Challenge>> {       
        @Override protected ObservableList<Challenge> call() throws Exception {
            for (int i = 0; i < 200; i++) {
                updateProgress(i, 180);
                Thread.sleep(3);
        ...
        String s1=s.replace("/","-");
        List<Challenge> list=serviceApprenant.displayChallengeByDate(Date.valueOf(s1),CurrentUser.getId());
        ChallengeData.addAll(list);           
        return ChallengeData;
        }
    } 
@FXML
   ...
   ChallengeData.clear();
        Task<ObservableList<Challenge>> task = new ListeChallangefxmlController.GetListCatByNom();
        p.progressProperty().bind(task.progressProperty());
        veil.visibleProperty().bind(task.runningProperty());
        p.visibleProperty().bind(task.runningProperty());
        NotifTable.itemsProperty().bind(task.valueProperty());
    ...
    FileChooser fileChooser = new FileChooser();
     fileChooser.setTitle("Export en PDF");
      ...
      if (file != null) {
          PdfListChallenge pdf = new PdfListChallenge(file.getPath());
